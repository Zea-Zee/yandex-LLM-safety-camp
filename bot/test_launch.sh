...
curl -X POST "$BOT_URL/webhook" \
     -H "Content-Type: application/json" \
     -d '{"update_id": 1, "message": {"message_id": 1, "from": {"id": 123, "is_bot": false, "first_name": "Test"}, "chat": {"id": 123, "type": "private"}, "date": 1758628880, "text": "/start"}}' | jq

# 5. Test webhook processing with regular message
echo -e "\n5. Testing regular message:"
curl -X POST "$BOT_URL/webhook" \
     -H "Content-Type: application/json" \
     -d '{"update_id": 2, "message": {"message_id": 2, "from": {"id": 123, "is_bot": false, "first_name": "Test"}, "chat": {"id": 123, "type": "private"}, "date": 1758628880, "text": "Привет! Как дела?"}}' | jq

# 6. Test webhook processing with test message
echo -e "\n6. Testing test message:"
curl -X POST "$BOT_URL/webhook" \
     -H "Content-Type: application/json" \
     -d '{"update_id": 3, "message": {"message_id": 3, "from": {"id": 123, "is_bot": false, "first_name": "Test"}, "chat": {"id": 123, "type": "private"}, "date": 1758628880, "text": "test"}}' | jq
